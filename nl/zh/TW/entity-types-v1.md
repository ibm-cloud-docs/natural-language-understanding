...
下列各表列出_第 1 版_ 實體類型系統中所使用的實體類型及子類型。
{: shortdesc}
...
對於使用_第 1 版_ 實體類型系統同時也支援關係的語言，關係結果包含的實體類型不同於實體結果中傳回的實體類型。若要傳回實體結果中的關係實體類型，您可以在 `entities` 特性的 `model` 選項中指定下列其中一個公用實體模型，以置換預設的實體偵測模型。
