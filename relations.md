...
The set of entities detected by the `relations` feature is different from the set of entities detected by the default model for the `entities` feature. You can specify one of the following `relations` models in the `model` parameter for the `entities` feature to override the default entity detection model.
